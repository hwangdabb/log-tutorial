...
step8_merge_and_export <- function(df_in, output_dir, country_codes) {
  ...
  # Note: Modified by human — fixed a bug where single-country data was not exported.
  #       In the single-country case, SEQIDs lack a country code prefix,
  #       so the entire dataframe is exported directly without prefix-based filtering.
  # Corrected logic to handle single-country export:
  if (length(country_codes) == 1) {
